...
sl = open("/home/christoph/sid/data/GBA2018/Berichte-Teile-A-B-C/260100023-01-2018-Land.xml") do file
        read(file, String);
end;
...
function allfiles(icds,opss;savePath="./",filename="data",dataDirectory="/home/christoph/sid/data/GBA2018/Berichte-Teile-A-B-C/")
...
function getaddress(filestring,vars = ["Name","Strasse","Hausnummer","Postleitzahl","Ort","IK","Standortnummer"])
    ...
    """
    if findfirst("<Standortkontaktdaten>",filestring)!=nothing
        kontaktStart = collect(findfirst("<Standortkontaktdaten>",filestring))[end]+1
        kontaktEnd = collect(findnext("</Standortkontaktdaten>",filestring,kontaktStart))[1]-1
...
function allfilesQI(ids,lbs;savePath="./",filename="dataQI",dataDirectory="/home/christoph/sid/data/GBA2018/Berichte-Teile-A-B-C/",varsString = ["EntwErg","QualBew","QualVgl"],varsFloat=["BErg","Erg","GG","BeobEreig","ErwEreig","RefBereich","VBBlow","VBBhigh","VBKlow","VBKhigh"])
    relevantFiles = filter(x -> endswith(x, "Land.xml"), readdir(dataDirectory))
...
function getQIData(s,id,varsString,varsFloat)
    ...
    abbreviations = Dict("EntwErg"=>"Entwicklung_Ergebnis_zum_vorherigen_Berichtsjahr","QualBew"=>"Qualitative_Bewertung_Berichtsjahr","QualVgl"=>"Qualitative_Bewertung_Vergleich_vorheriges_Berichtsjahr","DokRate"=>"Dokumentationsrate","BErg"=>"Bundesergebnis","VBBlow"=>"Vertrauensbereich_Untere_Grenze","VBBhigh"=>"Vertrauensbereich_Obere_Grenze","Erg"=>"Rechnerisches_Ergebnis","VBKlow"=>"Vertrauensbereich_Untere_Grenze","VBKhigh"=>"Vertrauensbereich_Obere_Grenze","GG"=>"Grundgesamtheit","BeobEreig"=>"Beobachtete_Ereignisse","ErwEreig"=>"Erwartete_Ereignisse","RefBereich"=>"Referenzbereich")
    #determine point where Dokumentationsraten end and actual quality indicators start
    temp0 = findfirst("</Dokumentationsraten>",s)
    if temp0!=nothing #this is the case if the hospital reported some quality indicators
        ...
        temp = findnext("<Ergebnis_ID>"*id,s,endDokRates)
        #println("temp: ",temp)
        if temp!=nothing #is the case if the id is found in the file
            ...
            startpoint = collect(findprev("<QS-Ergebnis>",s,position))[end]
            #println("Pos2: ",findnext("</QS-Ergebnis>",s,position))
            endpoint = collect(findnext("</QS-Ergebnis>",s,position))[1]
