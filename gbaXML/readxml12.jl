#to check:
# - Stimmt es, dass der "Nenner" in den QUalitätsberichten das ist was in späteren Jahren  "Grundgesamtheit ist und  "Zaehler" das ist was später "Beobachtete Ereignisse" sind?
# - Stimmt es, dass in den Qualitätsberichten der "Vertrauensbereich" das ist was später "Vertrauensbereich_Krankenhaus" ist (ich nehme das hier so an). (Es könnte theoretisch auch das sein was später "Vertrauensbereich_Bundesweit" ist.)
...
function allfiles(icds,opss;savePath="./",filename="data",dataDirectory="/home/christoph/sid/data/GBA2012/Berichte-KH")
    relevantFiles = filter(x -> endswith(x, "xml.xml"), readdir(dataDirectory))
    for (i,f) in enumerate(relevantFiles)
        if f in readdir(dataDirectory*"_Nachlieferung/")
            relevantFiles[i] = dataDirectory*"_Nachlieferung/"*f
        else
            relevantFiles[i] = dataDirectory*"/"*f
        end
    end
    df = DataFrame(name=Union{String,Missing}[],strasse=Union{String,Missing}[],hnr=Union{String,Missing}[],plz=Union{String,Missing}[],ort=Union{String,Missing}[],institutionenkennzeichen=Union{String,Missing}[],Standortnr=Union{String,Missing}[],traeger=Union{String,Missing}[],tArt=Union{String,Missing}[],betten=Union{Int64,Missing}[],faelleVollStationaer=Union{Int64,Missing}[],fealleTeilStationaer=Union{Int64,Missing}[],faelleAmbulant=Union{Int64,Missing}[],fullAdress=Union{String,Missing}[])
    for icd in icds
        df[!,Symbol(icd)] = Int64[]
    end
    for ops in opss
...
function extractVariable(fieldName,dataString; emptyIfMissing=false)
    """
...
function getaddress(filestring,vars = ["Name","Strasse","Hausnummer","Postleitzahl","Ort","IK","Standortnummer"])
    ...
    return extractVariable.(vars,kontaktData;emptyIfMissing=true)
end
...
@time allfiles(["O80","O81","O82","Z38.0","Z38.1","Z38.2","Z38.3","Z38.4","Z38.5","Z38.6"],["5-72","5-724","5-73","5-732.2","5-740","5-741","5-742","5-749","5-749.0"];savePath="./",filename="data",dataDirectory="/home/christoph/sid/data/GBA2012/Berichte-KH");
...
function allfilesQI(ids,lbs;savePath="./",filename="dataQI",dataDirectory="/home/christoph/sid/data/GBA2012/Berichte-LQS-AQUA",varsString = ["EntwErg","QualBew","QualVgl"],varsFloat=["BErg","Erg","GG","BeobEreig","ErwEreig","RefBereich","VBBlow","VBBhigh","VBKlow","VBKhigh"])
    ...
    for f in relevantFiles
        onefileQI!(f,df,ids,varsString,varsFloat,lbs)
    end
...
function getQIData(s,id,varsString,varsFloat)
    ...
    abbreviations = Dict("EntwErg"=>"Entwicklung_Ergebnis_zum_vorherigen_Berichtsjahr","QualBew"=>"Bewertung_Strukturierter_Dialog","QualVgl"=>"Vergleich_vorheriges_Berichtsjahr","DokRate"=>"Dokumentationsrate","BErg"=>"Bundesdurchschnitt","VBBlow"=>"Vertrauensbereich_Untere_Grenze","VBBhigh"=>"Vertrauensbereich_Obere_Grenze","Erg"=>"Ergebnis","VBKlow"=>"Vertrauensbereich_Untere_Grenze","VBKhigh"=>"Vertrauensbereich_Obere_Grenze","GG"=>"Nenner","BeobEreig"=>"Zaehler","ErwEreig"=>"Erwartete_Ereignisse","RefBereich"=>"Referenzbereich")
    ...
    if temp0!=nothing #this is the case if the hospital reported some quality indicators
        ...
        temp = findnext("<Kuerzel>"*id,s,endDokRates)
        #println("temp: ",temp)
        if temp!=nothing #is the case if the id is found in the file
            ...
            out1 = [extractVariableQI(abbreviations[var],extractVariableQI("Abgeschlossener_Strukt_Dialog",relevantString;emptyIfMissing=true)) for var in varsString]
            out2 = parseExtended.([extractVariableQI(abbreviations[var],extractVariableQI("Abgeschlossener_Strukt_Dialog",relevantString;emptyIfMissing=true)) for var in varsFloat[1:end-5]])
            ...
            out5 = parseExtended.(extractVariableQI.(["Vertrauensbereich_Untere_Grenze","Vertrauensbereich_Obere_Grenze"],extractVariableQI("Vertrauensbereich",relevantString;emptyIfMissing=true)))
...
@time allfilesQI(["11879","50103"],["09/3","PNEU"];savePath="./",filename="dataQI",dataDirectory="/home/christoph/sid/data/GBA2012/Berichte-LQS-AQUA",varsString = ["EntwErg","QualBew","QualVgl"],varsFloat=["BErg","Erg","GG","BeobEreig","ErwEreig","RefBereich","VBBlow","VBBhigh","VBKlow","VBKhigh"]);
